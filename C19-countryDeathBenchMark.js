$(document).ready(function(){
			...
			function plotData(row,index,color,subLevel){
				setTimeout(()=>{
					...
					for (country in doubleData) {
						if (doubleData.hasOwnProperty(country)) {
							if(counter == 0){
								...
								ctx.textAlign = 'right';
								ctx.fillText(country, 94, (countryCount*95)+37)
								ctx.fill();
								ctx.closePath();	
							}
							for (var i = 0; i < doubleData[country][counter]; i++) {
								plotData(countryCount,offsetCounter[country],colorArray[counter],0)
								plotData(countryCount,offsetCounter[country],colorArray[counter],1)
								plotData(countryCount,offsetCounter[country],colorArray[counter],2)
