...
var init = function(label){
	...
	const labelsArr = ["Kerala","Chhattisgarh","Uttarakhand","Odisha","Madhya Pradesh"]
	barArray = barArray.filter(state => labelsArr.indexOf(state['label'])>-1)
	console.log(barArray);
	...
	for (let counter = 0; counter < 2700; counter+=10) {
		ctx2.lineWidth =1;
		ctx2.beginPath();
		ctx2.font = '500 18px Montserrat'
		ctx2.textAlign = 'left';
		if(counter%20 === 0 && counter!==0){
			ctx2.fillText(counter+'K', 0 + 30, 6+(canHgt*(1-(counter*1000/heightFactor))));
			// ctx2.fill();
		}else if(counter===0){
			ctx2.fillText(counter+'K', 0 + 30, canHgt);
		}
		ctx2.closePath();
		...
		ctx2.beginPath();
		counter%20 === 0?
			ctx2.rect(0,(canHgt*(1-(counter*1000/heightFactor))),20,1)
			:ctx2.rect(0,(canHgt*(1-(counter*1000/heightFactor))),10,1);
		ctx2.fill();
		ctx2.closePath()
