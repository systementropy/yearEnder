...
function plotData(row,index,color,countryCount,counter){
	console.log(row,index,countryCount);
	setTimeout(()=>{
		console.log(row);
		ctx.beginPath();
		ctx.fillStyle =color;
		ctx.rect(
			(countryCount*150) + (index*10),
			(counter*80)+40,
			9,
			40
		);
...
function plotDaysText(row,index,color,label){
	if(label){
		setTimeout(()=>{
			ctx.beginPath();
			ctx.fillStyle =color;
			ctx.fillText(label,(row*150)+90,40+(index*10)),
			ctx.fill();
			ctx.closePath();
		},index*50)
	}
		...
		for (country in doubleData) {
			if (doubleData.hasOwnProperty(country)) {
				if(counter == 0){
					...
					ctx.textAlign = 'left';
					ctx.fillText(country, (countryCount*150)+40, 25)
					ctx.fill();
					ctx.closePath();	
				}
				offsetCounter[country] = 0;
				for (var i = 0; i < doubleData[country][counter]; i++) {
					plotData(countryCount,offsetCounter[country],colorArray[counter],countryCount,counter)
