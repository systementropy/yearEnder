$(document).ready(function(){
    ...
    function makeGraph(counter){
        if(counter<dailyCumulative[0].length){
            ...
            let activeNum = dailyCumulative[0][counter];
            let recvNum = dailyCumulative[1][counter];
			...
			activeNum>=100000
				?$('.confirmedData').text((activeNum/100000).toFixed(1)+'L')
				:activeNum>=1000?$('.confirmedData').text((activeNum/1000).toFixed(1)+'K'):$('.confirmedData').text(activeNum)
			recvNum>=100000
				?$('.recoveredData').text((recvNum/100000).toFixed(1)+'L')
    ...
    setTimeout(function () {
        ...
        ctx.fill();
        ctx.closePath();
        for (let counter = 0; counter < 500; counter+=25) {
            ctx.lineWidth =1;
            ctx.beginPath();
            ...
            if(counter===0){
				...
				ctx.fillText(counter+'L', canWidAvailable + 30, 15);
